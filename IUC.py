...
def generate_usernames():
    ...
    for _ in range(num_users):
        username = "".join(
            choices(string.ascii_lowercase + string.digits + "__", k=length)
        )
    ...
    def process_username(user, index):
        ...
        try:
            ...
            if user in content:
                ...
                status = "free" if find[1:4] == "url" else "taken"
        ...
        line_start = f"{result_area.index('end-2c linestart')}"
        line_end = f"{result_area.index('end-1c linestart')}"
        result_area.tag_add(status, line_start, line_end)

        # Define colors
        result_area.tag_config("free", foreground="green")
        result_area.tag_config("taken", foreground="red")
...
# Delay selector
tk.Label(input_frame, text="Delay between requests (seconds):", bg="#f0f0f0").grid(
...
delay_dropdown.grid(row=4, column=1, padx=5, pady=5)
...
tk.Label(
    app,
    text="Usernames List (you can drop your own list or generate one):",
    bg="#f0f0f0",
).pack(pady=5)
text_area = scrolledtext.ScrolledText(
    app, width=90, height=10, bg="#ffffff", fg="#000000"
)
text_area.pack(pady=5)

tk.Label(app, text="Notice: Blocked usernames may show as 'Free'", bg="#f0f0f0").pack(
    pady=5
)
...
result_area = scrolledtext.ScrolledText(
    app, width=90, height=10, bg="#ffffff", fg="#000000", state=tk.NORMAL
)
...
footer = tk.Label(
    app,
    text="© 2024 Asgard | Instagram: @3.9.9.6",
    bg="#f0f0f0",
    anchor="e",
    padx=10,
    pady=10,
